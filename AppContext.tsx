...
const DEFAULT_TRUST_MESSAGES: TrustMessage[] = [
...
interface AppContextType {
  ...
  // Banner Slider Dynamic Management
  bannerSlides: BannerSlide[];
  saveBannerSlide: (slide: BannerSlide) => Promise<void>;
  deleteBannerSlide: (slideId: string) => Promise<void>;

  // Trust Messages Auto Scroll Ticker Management
  trustMessages: TrustMessage[];
  saveTrustMessage: (msg: TrustMessage) => Promise<void>;
  deleteTrustMessage: (msgId: string) => Promise<void>;
}
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Banner Slides Dynamic State
  const [bannerSlides, setBannerSlides] = useState<BannerSlide[]>(() => {
    const saved = localStorage.getItem('iwc_banner_slides');
    return saved ? JSON.parse(saved) : DEFAULT_BANNER_SLIDES;
  });

  const saveBannerSlide = async (slide: BannerSlide) => {
    ...
    try {
      await FirebaseRepository.saveBannerSlide(slide);
      showToast('Banner slide updated in Firebase!');
    } catch (err) {
      console.warn('Local banner slide saved (offline mode).');
    }
  };

  const deleteBannerSlide = async (slideId: string) => {
    setBannerSlides(prev => prev.filter(s => s.id !== slideId));
    try {
      await FirebaseRepository.deleteBannerSlide(slideId);
      showToast('Banner slide removed.');
    } catch (err) {
      console.warn('Local banner slide deleted (offline mode).');
    }
  };

  // Trust Messages Dynamic Ticker State
  const [trustMessages, setTrustMessages] = useState<TrustMessage[]>(() => {
    const saved = localStorage.getItem('iwc_trust_messages');
    return saved ? JSON.parse(saved) : DEFAULT_TRUST_MESSAGES;
  });

  const saveTrustMessage = async (msg: TrustMessage) => {
    setTrustMessages(prev => {
    ...
    try {
      await FirebaseRepository.saveTrustMessage(msg);
      showToast('Trust message updated in Firebase!');
    } catch (err) {
      console.warn('Local trust message saved (offline mode).');
    }
  };

  const deleteTrustMessage = async (msgId: string) => {
    setTrustMessages(prev => prev.filter(m => m.id !== msgId));
    try {
      await FirebaseRepository.deleteTrustMessage(msgId);
      showToast('Trust message removed.');
    } catch (err) {
      console.warn('Local trust message deleted (offline mode).');
    }
  };
  ...
  useEffect(() => {
    ...
    // 7. Subscribe Banner Slides
    const unsubBanners = FirebaseRepository.subscribeBannerSlides((remoteBanners) => {
      if (remoteBanners && remoteBanners.length > 0) {
        setBannerSlides(remoteBanners);
      }
    });

    // 8. Subscribe Trust Messages
    const unsubTrustMsgs = FirebaseRepository.subscribeTrustMessages((remoteTrust) => {
      if (remoteTrust && remoteTrust.length > 0) {
        setTrustMessages(remoteTrust);
      }
    });

    // 9. Subscribe Realtime Auth User Profile
    ...
    const unsubAuth = onAuthStateChanged(auth, (fbUser) => {
      try {
        ...
        console.warn('Auth state subscription error:', err);
      }
    }, (err) => {
      console.warn('Firebase Auth State Changed Error:', err);
    });
  ...
  // Sync banner slides to LocalStorage
  useEffect(() => {
    localStorage.setItem('iwc_banner_slides', JSON.stringify(bannerSlides));
  }, [bannerSlides]);

  // Sync trust messages to LocalStorage
  useEffect(() => {
    localStorage.setItem('iwc_trust_messages', JSON.stringify(trustMessages));
  }, [trustMessages]);
  ...
  // Sync Dark/Light Theme with document element class & LocalStorage
  useEffect(() => {
    localStorage.setItem('iwc_dark_mode', String(darkMode));
    if (darkMode) {

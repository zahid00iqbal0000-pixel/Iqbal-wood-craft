...
import { Product, Order, CustomOrderRequest, PaymentAccountDetails, AiFaqItem, AiConversationLog, CustomerUser, BannerSlide, TrustMessage } from '../types';
...
const BANNER_SLIDES_COL = 'banner_slides';
const TRUST_MESSAGES_COL = 'trust_messages';
...
export const FirebaseRepository = {
  ...
  async clearAllProducts(): Promise<void> {
    try {
      ...
      const deletePromises = snapshot.docs.map(d => deleteDoc(doc(db, PRODUCTS_COL, d.id)));
      await Promise.all(deletePromises);
    } catch (err) {
      console.error('Firebase clearAllProducts Error:', err);
      throw err;
    }
  },
  ...
  subscribeAiFaqs(onUpdate: (faqs: AiFaqItem[]) => void) {
    ...
    return onSnapshot(q, (snapshot) => {
      ...
      console.warn('Realtime AI FAQs Sync Error:', err);
    });
  },
  ...
  subscribeAiConversations(onUpdate: (convs: AiConversationLog[]) => void) {
    ...
    return onSnapshot(q, (snapshot) => {
      ...
      console.warn('Realtime AI Conversations Sync Error:', err);
    });
  },
  ...
  subscribePaymentDetails(onUpdate: (details: PaymentAccountDetails) => void) {
    ...
    return onSnapshot(docRef, (snapshot) => {
      ...
      console.warn('Realtime Payment Details Sync Error:', err);
    });
  },
  ...
  // --- BANNER SLIDES ---
  subscribeBannerSlides(onUpdate: (slides: BannerSlide[]) => void) {
    const q = query(collection(db, BANNER_SLIDES_COL));
    return onSnapshot(q, (snapshot) => {
      if (snapshot.empty) {
        onUpdate([]);
        return;
      }
      const slides: BannerSlide[] = snapshot.docs.map(d => ({
        id: d.id,
        ...d.data()
      } as BannerSlide));
      onUpdate(slides);
    }, (err) => {
      console.warn('Realtime Banner Slides Sync Error:', err);
    });
  },

  async saveBannerSlide(slide: BannerSlide): Promise<void> {
    try {
      const docRef = doc(db, BANNER_SLIDES_COL, slide.id);
      await setDoc(docRef, slide, { merge: true });
    } catch (err) {
      console.error('Firebase saveBannerSlide Error:', err);
      throw err;
    }
  },

  async deleteBannerSlide(slideId: string): Promise<void> {
    try {
      await deleteDoc(doc(db, BANNER_SLIDES_COL, slideId));
    } catch (err) {
      console.error('Firebase deleteBannerSlide Error:', err);
      throw err;
    }
  },

  // --- TRUST MESSAGES ---
  subscribeTrustMessages(onUpdate: (messages: TrustMessage[]) => void) {
    const q = query(collection(db, TRUST_MESSAGES_COL));
    return onSnapshot(q, (snapshot) => {
      if (snapshot.empty) {
        onUpdate([]);
        return;
      }
      const msgs: TrustMessage[] = snapshot.docs.map(d => ({
        id: d.id,
        ...d.data()
      } as TrustMessage));
      onUpdate(msgs);
    }, (err) => {
      console.warn('Realtime Trust Messages Sync Error:', err);
    });
  },

  async saveTrustMessage(msg: TrustMessage): Promise<void> {
    try {
      const docRef = doc(db, TRUST_MESSAGES_COL, msg.id);
      await setDoc(docRef, msg, { merge: true });
    } catch (err) {
      console.error('Firebase saveTrustMessage Error:', err);
      throw err;
    }
  },

  async deleteTrustMessage(msgId: string): Promise<void> {
    try {
      await deleteDoc(doc(db, TRUST_MESSAGES_COL, msgId));
    } catch (err) {
      console.error('Firebase deleteTrustMessage Error:', err);
      throw err;
    }
  }
};
